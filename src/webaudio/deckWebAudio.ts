...
CONTEXT.audioWorklet.addModule('/worklet/webAudio.js')
...
const createPositionTracker = (buffer: AudioBuffer): AudioBufferSourceNode => {
  const counterBuffer = CONTEXT.createBuffer(1, buffer.length, CONTEXT.sampleRate)
...
export class Deck {
  ...
  constructor () {
    ...
    this.compressor.connect(this.audioAnalyser)
    ...
    this.loadAndPlayTrack = (buffer: AudioBuffer, speed: number, position: number) => {
      ...
      this.positionReporter = new AudioWorkletNode(CONTEXT, 'position-reporting-processor')
      this.positionReporter.port.onmessage = (e) => {
        if (this.playbackState === PlaybackStates.PLAYING && this.updatePosition) {
    ...
    this.play = () => {
      if (this.loadedTrack) {
        ...
        if (this.positionTracker) this.positionTracker.playbackRate.value = this.playbackRate
        this.playbackState = PlaybackStates.PLAYING
        this.setPlaybackState && this.setPlaybackState(PlaybackStates.PLAYING)
        this.gainNode.gain.value = ZERO
        this.gainNode.gain.exponentialRampToValueAtTime(this.currentVolume, CONTEXT.currentTime + FADE_IN_OUT_TIME)
      }
    }
    this.pause = () => {
      if (this.loadedTrack) {
        ...
        this.gainNode.gain.exponentialRampToValueAtTime(ZERO, CONTEXT.currentTime + FADE_IN_OUT_TIME)
        setTimeout(() => {
          if (this.playbackState === PlaybackStates.PAUSED) {
            if (this.loadedTrack) this.loadedTrack.playbackRate.value = isFireFox ? ZERO : 0
            if (this.positionTracker) this.positionTracker.playbackRate.value = isFireFox ? ZERO : 0
          }
    ...
    this.handleJogWheel = (nextValue: number) => {
      if (this.loadedTrack && this.playbackState === PlaybackStates.PLAYING) {
        ...
        if (this.positionTracker) this.positionTracker.playbackRate.cancelScheduledValues(CONTEXT.currentTime)
        this.loadedTrack.playbackRate.linearRampToValueAtTime(this.computedPlaybackSpeed, CONTEXT.currentTime + 0.1)
        if (this.positionTracker) this.positionTracker.playbackRate.linearRampToValueAtTime(this.computedPlaybackSpeed, CONTEXT.currentTime + 0.1)
      } else {
    ...
    this.setPlayBackSpeed = (speed: number) => {
      ...
      if (this.loadedTrack) {
        ...
        if (this.playbackState === PlaybackStates.PLAYING) {
          ...
          if (this.positionTracker) this.positionTracker.playbackRate.value = newSpeed
        }
      }
    }
    this.cuePoint = null
    ...
    this.setFilter = (nextValue: number) => {
      ...
      if (isLowPass) {
        this.highPassFilter.frequency.exponentialRampToValueAtTime(25, CONTEXT.currentTime + FADE_IN_OUT_TIME)
        this.highPassFilter.Q.exponentialRampToValueAtTime(ZERO, CONTEXT.currentTime + FADE_IN_OUT_TIME)
        this.lowPassFilter.Q.exponentialRampToValueAtTime(4, CONTEXT.currentTime + FADE_IN_OUT_TIME)
        const value = Math.pow((nextValue + 50) / 50, 5) * 19950 + 25
        this.lowPassFilter.frequency.exponentialRampToValueAtTime(value, CONTEXT.currentTime + FADE_IN_OUT_TIME)
      } else {
        this.lowPassFilter.frequency.exponentialRampToValueAtTime(20000, CONTEXT.currentTime + FADE_IN_OUT_TIME)
        this.lowPassFilter.Q.exponentialRampToValueAtTime(ZERO, CONTEXT.currentTime + FADE_IN_OUT_TIME)
        this.highPassFilter.Q.exponentialRampToValueAtTime(6, CONTEXT.currentTime + FADE_IN_OUT_TIME)
        const exponential = Math.pow(Math.abs(nextValue / 50), 4)
        const valueToSave = (exponential + (isLowPass ? 50 : 0)) * 15000 + 25
        this.highPassFilter.frequency.exponentialRampToValueAtTime(valueToSave, CONTEXT.currentTime + FADE_IN_OUT_TIME)
      }
    }
    this.metaData = {}
    this.setVolume = (nextVolume: number) => {
      ...
      this.gainNode.gain.cancelScheduledValues(0)
      this.gainNode.gain.exponentialRampToValueAtTime(value, CONTEXT.currentTime + FADE_IN_OUT_TIME)
    }
    this.setMasterGain = (nextVolume: number) => {
      const value = nextVolume > 1 ? 1 : nextVolume < ZERO ? ZERO : nextVolume
      this.masterGain.gain.cancelScheduledValues(0)
      this.masterGain.gain.exponentialRampToValueAtTime(value, CONTEXT.currentTime + FADE_IN_OUT_TIME)
    }
