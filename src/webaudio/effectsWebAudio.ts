    ...
    if (lastTap) {
      ...
      if (time < 5) {
        ...
        delay.delayTime.linearRampToValueAtTime(
          time,
          CONTEXT.currentTime + DELAY_CHANGE_TIME
        );
      }
    }
    lastTap = now;
...
export class Effects {
  ...
  constructor() {
    ...
    this.handleX2 = () => {
      ...
      !isFireFox &&
        this.delay.delayTime.cancelAndHoldAtTime(CONTEXT.currentTime);
    ...
    this.changeDeckAInput = (value: number) => {
      ...
      this.deckAInput.gain.linearRampToValueAtTime(
        nextValue,
        CONTEXT.currentTime + FADE_IN_OUT_TIME
      );
    };
    this.changeDeckBInput = (value: number) => {
      ...
      this.deckBInput.gain.linearRampToValueAtTime(
        nextValue,
        CONTEXT.currentTime + FADE_IN_OUT_TIME
      );
    };
...
EFFECTS.output.connect(compressor);
compressor.connect(audioRouter.cueMixVolume);
